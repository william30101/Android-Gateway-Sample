 ...
 * {@link gatewaysample.kii.com.gateway_agent_androidsample.mqtt.Connection} objects so created clients survive, the destruction of the
 * singleton {@link gatewaysample.kii.com.gateway_agent_androidsample.mqtt.Connections} object.
 *
 */
public class Persistence extends SQLiteOpenHelper implements BaseColumns {
   ...
   * @throws gatewaysample.kii.com.gateway_agent_androidsample.mqtt.PersistenceException If storing the data fails
   */
  public void persistConnection(gatewaysample.kii.com.gateway_agent_androidsample.mqtt.Connection connection) throws gatewaysample.kii.com.gateway_agent_androidsample.mqtt.PersistenceException {
    ...
    if (newRowId == -1) {
      throw new gatewaysample.kii.com.gateway_agent_androidsample.mqtt.PersistenceException("Failed to persist connection: " + connection.handle());
    }
   ...
   * @param context Context for creating {@link gatewaysample.kii.com.gateway_agent_androidsample.mqtt.Connection} objects
   * @return list of connections that have been restored
   * @throws gatewaysample.kii.com.gateway_agent_androidsample.mqtt.PersistenceException if restoring connections fails, this is thrown
   */
  public List<gatewaysample.kii.com.gateway_agent_androidsample.mqtt.Connection> restoreConnections(Context context) throws gatewaysample.kii.com.gateway_agent_androidsample.mqtt.PersistenceException
  {
    //columns to return
    ...
    ArrayList<gatewaysample.kii.com.gateway_agent_androidsample.mqtt.Connection> list = new ArrayList<gatewaysample.kii.com.gateway_agent_androidsample.mqtt.Connection>(c.getCount());
    gatewaysample.kii.com.gateway_agent_androidsample.mqtt.Connection connection = null;
    for (int i = 0; i < c.getCount(); i++) {
      if (!c.moveToNext()) { //move to the next item throw persistence exception, if it fails
        throw new gatewaysample.kii.com.gateway_agent_androidsample.mqtt.PersistenceException("Failed restoring connection - count: " + c.getCount() + "loop iteration: " + i);
      }
      ...
      connection = gatewaysample.kii.com.gateway_agent_androidsample.mqtt.Connection.createConnection(clientID, host, port, context, ssl);
   ...
   */
  public void deleteConnection(gatewaysample.kii.com.gateway_agent_androidsample.mqtt.Connection connection) {
    SQLiteDatabase db = getWritableDatabase();
